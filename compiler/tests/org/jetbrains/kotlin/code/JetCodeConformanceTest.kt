...
public class JetCodeConformanceTest : TestCase() {
    ...
    public fun testNoBadSubstringsInProjectCode() {
        ...
        val foundFiles = (1..tests.size()).map { ArrayList<File>() }
        ...
        for (sourceFile in FileUtil.findFilesByMask(SOURCES_FILE_PATTERN, File("."))) {
            ...
            for ((test, result) in tests.zip(foundFiles)) {
                if (test.filter(source)) result.add(sourceFile)
            }
        }

        if (foundFiles.flatten().isNotEmpty()) {
            fail(StringBuilder {
                for ((test, result) in tests.zip(foundFiles)) {
                    if (result.isNotEmpty()) {
                        append(test.message.format(result.size(), result.joinToString("\n")))
                        appendln()
                        appendln()
                    }
                }
